...
class Config {
    ...
    function __construct() {
        $config_location = realpath(dirname(__FILE__).'/../config.json');
    ...
    /**
     * Returns the Directory, where the config.json file has been stored.
     * @return String The real path to th Directory containing config.json.
     */
    public function config_dir() {
        return dirname(realpath(dirname(__FILE__).'/../config.json'));
    }
}
...
class Service_Auth {
    ...
    function __construct() {
        ...
        $auth_file = realpath($_CONFIG->config_dir() . '/' . $_CONFIG->auth_json_file);
        $auth_p12_file = realpath($_CONFIG->config_dir() . '/' . $_CONFIG->auth_certificate_p12);
...
class State {
     ...
     */
    function __construct() {
        self::$STATE_FILE = dirname(__FILE__).'/../private/state.json';
        self::$changed = false;
